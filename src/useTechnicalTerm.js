...
export const useTechnicalTerm = () => {
  const [technicalTerm, setTechnicalTerm] = useState([
    ...
    },
    {
      ...
      explanation:
        'アラキドン酸はシクロオキシゲナーゼCOXの働きによってプロスタノイドに代謝される。NSAIDsによって活動が阻害され、特にアスピリンは不可逆的にCOXを阻害する。COX1は非誘導型（ほぼ全ての細胞に常時発現している）で止血作用・生理的な働きを行うプロスタグランジン(TXA₂など)を合成する。COX2は誘導型（炎症箇所で刺激依存性に発現する）で炎症など病的な状態ではたらくプロスタグランジン(PGI₂など)の合成を行う。',
    },
    // {
    //   term:[''],
    //   explanation:'',
